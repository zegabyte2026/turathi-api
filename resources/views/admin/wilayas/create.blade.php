...
@section('title', __('admin.wilayas_new'))
...
<div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8" style="background-color: #0D211D;">
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.wilayas.index') }}" class="inline-flex items-center gap-2 text-sm font-medium rounded-lg px-4 py-2 transition-colors" style="color: #F3E9CF; background-color: #3E8E7E;">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                {{ __('admin.common_back') }}
            </a>
            <h1 class="text-2xl font-bold" style="color: #F3E9CF;">{{ __('admin.wilayas_new') }}</h1>
        </div>
        ...
        <form action="{{ route('admin.wilayas.store') }}" method="POST" enctype="multipart/form-data" class="rounded-xl p-6 space-y-6" style="background-color: #3E8E7E;">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="name[fr]" id="name_fr" value="{{ old('name.fr') }}" class="w-full rounded-lg px-3 py-2 text-sm border-0 focus:ring-2" style="background-color: #0D211D; color: #F3E9CF;" required>
                        @error('name.fr')
                    ...
                    <div>
                        ...
                        <input type="text" name="name[ar]" id="name_ar" value="{{ old('name.ar') }}" class="w-full rounded-lg px-3 py-2 text-sm border-0 focus:ring-2" style="background-color: #0D211D; color: #F3E9CF;" dir="rtl" required>
                        @error('name.ar')
                            <p class="mt-1 text-xs" style="color: #F3E9CF;">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" name="name[en]" id="name_en" value="{{ old('name.en') }}" class="w-full rounded-lg px-3 py-2 text-sm border-0 focus:ring-2" style="background-color: #0D211D; color: #F3E9CF;" required>
                        @error('name.en')
                            <p class="mt-1 text-xs" style="color: #F3E9CF;">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex items-center gap-3 pt-4">
                ...
                <a href="{{ route('admin.wilayas.index') }}" class="inline-flex items-center gap-2 text-sm font-medium rounded-lg px-6 py-2.5 transition-colors" style="color: #F3E9CF; background-color: #0D211D; border: 1px solid #3E8E7E;">
