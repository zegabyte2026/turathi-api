...
@section('title', __('admin.sidebar_objets'))
...
<div class="min-h-screen bg-[#F3E9CF] py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-[#0D211D]">{{ __('admin.sidebar_objets') }}</h1>
            <a href="{{ route('admin.objets.create') }}" class="inline-flex items-center gap-2 text-sm font-semibold rounded-lg px-5 py-2.5 bg-[#3E8E7E] text-white hover:bg-[#2d7a6a] transition-colors">
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.endroits_title') }}</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.sidebar_sites') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($objets as $objet)
                        @php
                            $hasMedia = (!empty($objet->images) && is_array($objet->images) && count($objet->images)) || (!empty($objet->audio_paths) && is_array($objet->audio_paths) && count($objet->audio_paths));
                        @endphp
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-sm text-gray-400">{{ $objet->id }}</td>
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.objets.show', $objet) }}" class="text-sm font-semibold text-[#0D211D] hover:underline">{{ $objet->title['fr'] ?? '—' }}</a>
                                @if(!$hasMedia)
                                    <span class="ml-2 inline-block px-1.5 py-0.5 text-[10px] font-bold rounded bg-yellow-100 text-yellow-700">INCOMPLET</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                @if($objet->endroit)
                                    <a href="{{ route('admin.endroits.show', $objet->endroit) }}" class="hover:underline">{{ $objet->endroit->title['fr'] ?? '—' }}</a>
                                @else
                                    <span class="text-gray-300">—</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                @if($objet->endroit && $objet->endroit->site)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('admin.objets.show', $objet) }}" class="p-1.5 rounded-lg hover:bg-gray-100 transition-colors" title="{{ __('admin.common_view') }}">
                                        <svg class="w-4 h-4 text-[#3E8E7E]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <a href="{{ route('admin.objets.edit', $objet) }}" class="p-1.5 rounded-lg hover:bg-gray-100 transition-colors" title="{{ __('admin.common_edit') }}">
                                    ...
                                    <form action="{{ route('admin.objets.destroy', $objet) }}" method="POST" onsubmit="return confirm('{{ __('admin.common_confirm_delete') }}');">
                                        @csrf @method('DELETE')
