...
<div class="min-h-screen py-8 px-4 sm:px-6 lg:px-8" style="background-color: #0D211D;">
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.objets.index') }}" class="inline-flex items-center gap-2 text-sm font-medium rounded-lg px-4 py-2 transition-colors" style="color: #F3E9CF; background-color: #3E8E7E;">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                Retour
            </a>
            <h1 class="text-2xl font-bold" style="color: #F3E9CF;">Nouvel Objet</h1>
        </div>
        ...
        <form action="{{ route('admin.objets.store') }}" method="POST" enctype="multipart/form-data" class="rounded-xl p-6 space-y-6" style="background-color: #3E8E7E;">
            @csrf

            <div>
                <label for="endroit_id" class="block text-sm font-semibold mb-1" style="color: #F3E9CF;">Endroit parent</label>
                <select name="endroit_id" id="endroit_id" class="w-full rounded-lg px-3 py-2 text-sm border-0 focus:ring-2" style="background-color: #0D211D; color: #F3E9CF;" required>
                    <option value="">— Sélectionner un endroit —</option>
                    @foreach($endroits as $endroit)
                        <option value="{{ $endroit->id }}" {{ old('endroit_id') == $endroit->id ? 'selected' : '' }}>
                            {{ $endroit->title['fr'] ?? '—' }} — {{ $endroit->site->name['fr'] ?? '' }}
                        </option>
                    @endforeach
                </select>
                @error('endroit_id')
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        @error('title.ar') <p class="mt-1 text-xs" style="color: #F3E9CF;">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        @error('title.en') <p class="mt-1 text-xs" style="color: #F3E9CF;">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        @error('description.ar') <p class="mt-1 text-xs" style="color: #F3E9CF;">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        @error('description.en') <p class="mt-1 text-xs" style="color: #F3E9CF;">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="materiau" id="materiau" value="{{ old('materiau') }}" placeholder="Ex: Pierre calcaire, Bois, Céramique..." class="w-full rounded-lg px-3 py-2 text-sm border-0 focus:ring-2" style="background-color: #0D211D; color: #F3E9CF;">
                </div>
                <div>
                    <label for="periode" class="block text-sm font-semibold mb-1" style="color: #F3E9CF;">Période</label>
                    <input type="text" name="periode" id="periode" value="{{ old('periode') }}" placeholder="Ex: XVe siècle, Période ottomane..." class="w-full rounded-lg px-3 py-2 text-sm border-0 focus:ring-2" style="background-color: #0D211D; color: #F3E9CF;">
                </div>
                <div>
                    <label for="dimensions" class="block text-sm font-semibold mb-1" style="color: #F3E9CF;">Dimensions</label>
                    <input type="text" name="dimensions" id="dimensions" value="{{ old('dimensions') }}" placeholder="Ex: 120cm × 80cm × 45cm" class="w-full rounded-lg px-3 py-2 text-sm border-0 focus:ring-2" style="background-color: #0D211D; color: #F3E9CF;">
                </div>
            </div>

            <div>
                <label for="images" class="block text-sm font-semibold mb-1" style="color: #F3E9CF;">Photos</label>
                ...
                @error('images') <p class="mt-1 text-xs" style="color: #F3E9CF;">{{ $message }}</p> @enderror
            </div>

            <div class="space-y-4">
                <h3 class="text-sm font-bold uppercase tracking-wider" style="color: #F3E9CF;">Audio par langue</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', 1) ? 'checked' : '' }} class="w-4 h-4 rounded border-0 focus:ring-2" style="background-color: #0D211D; color: #3E8E7E;">
            ...
            <div class="flex items-center gap-3 pt-4">
                ...
                <a href="{{ route('admin.objets.index') }}" class="inline-flex items-center gap-2 text-sm font-medium rounded-lg px-6 py-2.5 transition-colors" style="color: #F3E9CF; background-color: #0D211D; border: 1px solid #3E8E7E;">
                    Annuler
                </a>
            </div>
        </form>
