...
@section('title', __('admin.users_edit_prefix') . $user->name)
...
<div class="min-h-screen bg-[#F3E9CF] p-6">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <h1 class="text-2xl font-bold text-[#0D211D] mb-6">{{ __('admin.users_edit_title') }}</h1>
            ...
            <form action="{{ route('admin.users.update', $user) }}" method="POST">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        <label for="password" class="block text-sm font-semibold text-[#0D211D] mb-1">{{ __('admin.users_password_new') }}</label>
                        <input type="password" name="password" id="password"
                               placeholder="{{ __('admin.users_password_placeholder') }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#3E8E7E] focus:border-[#3E8E7E] outline-none transition">
                    </div>

                    <div>
                        <label for="password_confirmation" class="block text-sm font-semibold text-[#0D211D] mb-1">{{ __('admin.users_password_confirm') }}</label>
                        <input type="password" name="password_confirmation" id="password_confirmation"
                               placeholder="{{ __('admin.users_password_confirm_placeholder') }}"
                               class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#3E8E7E] focus:border-[#3E8E7E] outline-none transition">
                        <p class="mt-1 text-xs text-gray-500">{{ __('admin.users_password_hint') }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="role" id="role" required
                            ...
                            <option value="local_admin" {{ old('role', $user->role) === 'local_admin' ? 'selected' : '' }}>{{ __('admin.users_local_admin') }}</option>
                            <option value="super_admin" {{ old('role', $user->role) === 'super_admin' ? 'selected' : '' }}>{{ __('admin.users_super_admin') }}</option>
                        </select>
                    </div>

                    <div id="site-field" class="{{ old('role', $user->role) === 'super_admin' ? 'hidden' : '' }}">
                        ...
                        <select name="site_id" id="site_id"
                            ...
                            @foreach($sites as $site)
                                <option value="{{ $site->id }}" {{ old('site_id', $user->sites->first()?->id) == $site->id ? 'selected' : '' }}>
                ...
                <div class="mt-6 flex space-x-3">
                    <button type="submit"
                            class="bg-[#B85C38] text-white px-6 py-2.5 rounded-lg hover:bg-[#9E4E2E] transition font-semibold shadow-md text-sm">
